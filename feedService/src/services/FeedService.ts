import { SvcReturnType, PaginationPost, PaginationComment, PaginationSavedPost, PaginationReports, PaginationSearchPost } from '../constants/SvcTypes';
...
import { getUsersBySearchText } from '../util/userClientFunctions';
...
class FeedService implements IFeedService {
  ...
  async searchPost(searchText: string, page: number, startDate?: string, endDate?: string): SvcReturnType<PaginationSearchPost> {
    try {
      const startIndex = (page - 1) * LIMIT
      ...
      const postIds = posts.map(post => post._id)
      const likes = await this.feedRepo.getLikes(postIds, 'post')
      const users = await getUsersBySearchText(searchText, page)
