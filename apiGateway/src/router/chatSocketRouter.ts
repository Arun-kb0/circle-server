...
import { answerCall, callUser, editMessage, handleSignal, iceCandidate, joinCallRoom, joinRoom, joinUserRoom, leaveCall, messageDeleted, messageTyping, messageTypingStopped, sendMessage } from '../controller/chatSocketController'
...
const chatSocketRouter = (socket: Socket) => {
  ...
  socket.on(SocketEvents.messageTyping, (message) => messageTyping(socket, message))
  socket.on(SocketEvents.messageTypingStopped, (message) => messageTypingStopped(socket, message))
